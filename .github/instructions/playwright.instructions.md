...
- **Clarity**: Use descriptive test and step titles that clearly state the intent. Add comments only to explain complex logic or non-obvious interactions; never restate the test step or assertion.
- **Formatting**: Use the repository's TypeScript conventions (consistent indentation, semicolons, single quotes, and trailing commas in multiline structures). ESLint checks the TypeScript quality baseline.
...
- [ ] Code is properly formatted and comments explain intent rather than restating the test
